<div class="overflow-x-auto bg-white shadow-lg rounded-b border p-4">
    <div class="p-6">
        ...
        <table class="min-w-max text-xs border-collapse w-full">
            ...
            <tbody>
                ...
                <tr class="font-bold text-white">
                    ...
                    $bgColor = $balance < 0 ? 'bg-red-600' : 'bg-indigo-50 text-indigo-700' ; @endphp <td class="border text-center {{ $bgColor }}">{{ $balance }}</td>
                        @endforeach
                        @php
                        $totalBalance = $fluidRecords->sum(fn($r) => $r->totalCairanMasuk() - $r->totalCairanKeluar());
                        $totalBgColor = $totalBalance < 0 ? 'bg-red-600' : 'bg-indigo-50 text-indigo-700 font-semibold' ; @endphp <td class="border text-center {{ $totalBgColor }}">{{ $totalBalance }}</td>
                </tr>
            </tbody>
        </table>
    </div>
