        ...
        <tbody>
            ...
            <tr class="{{ $loop->even ? 'bg-gray-50' : 'bg-white' }}">
                ...
                elseif($field == 'ph') $badge = ($value < 7.35 || $value> 7.45) ? 'bg-red-100 text-red-700 font-semibold' : 'bg-green-100 text-green-800';
                    elseif($field == 'sao2') $badge = $value < 95 ? 'bg-red-100 text-red-700 font-semibold' : 'bg-green-100 text-green-800' ; @endphp <td class="px-4 py-2 text-center {{ $badge }}">{{ $value ?? '-' }}</td>
                        @endforeach
            </tr>
            @endforeach
        </tbody>
