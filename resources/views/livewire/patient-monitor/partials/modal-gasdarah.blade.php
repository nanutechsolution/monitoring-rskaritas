            ...
            <div class="px-6 py-5 overflow-y-auto">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-5 gap-y-6">
                    <div class="sm:col-span-2 md:col-span-4">
                        <label for="form_taken_at_gas" class="{{ $labelModalClasses }} mb-1">Waktu Pengambilan Sampel</label>
                        <input id="form_taken_at_gas" type="datetime-local" x-model="taken_at"
                               class="{{ $inputModalClasses }}">
                        @error('taken_at') <span class="{{ $errorModalClasses }}">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div>
                        <label for="form_gas_{{ $field['id'] }}" class="{{ $labelModalClasses }}">{{ $field['label'] }}
                            @if(isset($field['unit']))<span class="text-xs text-gray-500 dark:text-gray-400">({{ $field['unit'] }})</span>@endif
                        </label>
                        <input id="form_gas_{{ $field['id'] }}" type="number"
                               ...
                               class="{{ $inputModalClasses }}">
                        @error($field['id']) <span class="{{ $errorModalClasses }}">{{ $message }}</span> @enderror
                    </div>
                    @endforeach
                </div>
            </div>
