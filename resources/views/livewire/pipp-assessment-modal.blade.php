...
<div class="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm">
    ...
    behavioral_state:      @entangle('behavioral_state').defer,
    max_heart_rate:        @entangle('max_heart_rate').defer,
    min_oxygen_saturation: @entangle('min_oxygen_saturation').defer,
    brow_bulge:            @entangle('brow_bulge').defer,
    eye_squeeze:           @entangle('eye_squeeze').defer,
    nasolabial_furrow:     @entangle('nasolabial_furrow').defer,

    // 2. Buat getter untuk menghitung total dari variabel Alpine (bukan $wire)
    get totalScore() {
        ...
        <form wire:submit.prevent="saveScore">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 border-t border-gray-200 pt-4">
                ...
                '0: ≥ 36 mgg', '1: 32–35 mgg + 6h', '2: 28–31 mgg + 6h', '3: < 28 mgg' ]], ['behavioral_state', 'Perilaku Bayi (15 detik)' , [ '0: Aktif/bangun, mata terbuka' , '1: Diam/bangun, mata terbuka/tertutup' , '2: Aktif/tidur, mata tertutup' , '3: Tenang/tidur, gerak minimal' ]], ['max_heart_rate', 'Laju Nadi Maks (peningkatan)' , [ '0: 0–4 dpm (Skor 0)' , '1: 5–14 dpm (Skor 1)' , '2: 15–24 dpm (Skor 2)' , '3: ≥25 dpm (Skor 3)' ]], ['min_oxygen_saturation', 'Saturasi O₂ Minimum (penurunan)' , [ '0: 92–100% (Skor 0)' , '1: 89–91% (Skor 1)' , '2: 85–88% (Skor 2)' , '3: <85% (Skor 3)' ]], ['brow_bulge', 'Tarikan Alis (% waktu)' , [ '0: Tidak ada (<9%)' , '1: Minimum (10–39%)' , '2: Sedang (40–69%)' , '3: Maksimum (≥70%)' ]], ['eye_squeeze', 'Kerutan Mata (% waktu)' , [ '0: Tidak ada (<9%)' , '1: Minimum (10–39%)' , '2: Sedang (40–69%)' , '3: Maksimum (≥70%)' ]], ['nasolabial_furrow', 'Alur Nasolabial (% waktu)' , [ '0: Tidak ada (<9%)' , '1: Minimum (10–39%)' , '2: Sedang (40–69%)' , '3: Maksimum (≥70%)' ]] ] as [$id, $label, $options]) <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ $label }}</label>
                    <select wire:model="{{ $id }}" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
