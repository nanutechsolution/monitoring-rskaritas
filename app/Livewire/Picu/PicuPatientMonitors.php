...
use App\Models\PicuPatientDevice;
...
class PicuPatientMonitors extends Component
{
    ...
    public ?PicuMonitoringCycle $currentCycle = null;
    ...
    public function confirmRemoveDevice($deviceId)
    {
        ...
        $device = PicuPatientDevice::find($deviceId);
    ...
    public function mount(string $no_rawat): void
    {
        ...
        $this->currentCycle = PicuMonitoringCycle::where('no_rawat', $this->no_rawat)
    ...
    public function loadPatientDevices(): void
    {
        ...
        $this->patientDevices = PicuPatientDevice::with(['installer', 'remover'])
            ->where('no_rawat', operator: $this->no_rawat)
    ...
    private function reloadRepeaterNames()
    {
        ...
        if ($this->currentCycleId) {
            $currentRecordIds = PicuMonitoringRecord::where('monitoring_cycle_id', $this->currentCycleId)->pluck('id');

            if ($currentRecordIds->isNotEmpty()) {
                $uniqueInfusionNames = \App\Models\PicuParenteralIntake::whereIn('monitoring_record_id', $currentRecordIds)
                    ->distinct()
                    ->pluck('name');
                $uniqueEnteralNames = \App\Models\PicuEnteralIntake::whereIn('monitoring_record_id', $currentRecordIds)
                    ->distinct()
                    ->pluck('name');
            }
        }
        ...
        if ($uniqueInfusionNames->isEmpty() && $uniqueEnteralNames->isEmpty()) {
            ...
            $previousCycle = PicuMonitoringCycle::where('no_rawat', $this->no_rawat)
                ->where('start_time', $previousCycleStartTime)
                ->first();

            // Jika siklus sebelumnya ADA, ambil nama dari sana
            if ($previousCycle) {
                $previousRecordIds = PicuMonitoringRecord::where('monitoring_cycle_id', $previousCycle->id)->pluck('id');

                if ($previousRecordIds->isNotEmpty()) {
                    // Hanya ambil jika nama saat ini benar-benar kosong
                    if ($uniqueInfusionNames->isEmpty()) {
                        $uniqueInfusionNames = \App\Models\PicuParenteralIntake::whereIn('monitoring_record_id', $previousRecordIds)
                            ->distinct()
                            ->pluck('name');
                    }
                    if ($uniqueEnteralNames->isEmpty()) {
                        $uniqueEnteralNames = \App\Models\PicuEnteralIntake::whereIn('monitoring_record_id', $previousRecordIds)
                            ->distinct()
                            ->pluck('name');
                    }
                }
    ...
    protected $messages = [
        ...
        'record_time.required' => 'Waktu pencatatan harus diisi.',
    ];
    ...
    public function loadRecords(): void
    {
        ...
        $currentCycle = PicuMonitoringCycle::where('no_rawat', $this->no_rawat)
            ->where('start_time', $cycleStartTime)
            ->first();
        $previousCycleStartTime = $cycleStartTime->copy()->subDay();
        $previousCycle = PicuMonitoringCycle::where('no_rawat', $this->no_rawat)
        ...
        if ($currentCycle) {
            ...
            $this->medications = MedicationPicu::with('pegawai')->where('monitoring_cycle_id', $currentCycle->id)->orderBy('given_at', 'desc')->get();
            $this->bloodGasResults = BloodGasResultPicu::with('pegawai')->where('monitoring_cycle_id', $currentCycle->id)->orderBy('taken_at', 'desc')->get();
            // Ambil SEMUA record siklus ini untuk kalkulasi total
            $allCycleRecords = PicuMonitoringRecord::with('parenteralIntakes', 'enteralIntakes', 'pegawai')
                ->where('monitoring_cycle_id', $currentCycle->id)
                ->get();
            $this->recentMedicationNames = MedicationPicu::where('monitoring_cycle_id', $currentCycle->id)
                ->distinct()
            ...
            $uniqueInfusionNames = \App\Models\PicuParenteralIntake::whereIn('monitoring_record_id', $recordIds)
                ->distinct()
                ->pluck('name');
            $uniqueEnteralNames = \App\Models\PicuEnteralIntake::whereIn('monitoring_record_id', $recordIds)
                ->distinct()
                ->pluck('name');
     ...
     */
    private function calculateAndSaveBalance(PicuMonitoringCycle $cycle)
    {
        $records = PicuMonitoringRecord::with('parenteralIntakes')->where('monitoring_cycle_id', $cycle->id)->get();
    ...
    public function saveDailyIwl()
    {
        if ($this->currentCycleId) {
            ...
            $cycle = PicuMonitoringCycle::find($this->currentCycleId);
    ...
    public function loadMedicationsOnly()
    {
        if ($this->currentCycleId) {
            ...
            $this->medications = MedicationPicu::with('pegawai')
                ->where('monitoring_cycle_id', $this->currentCycleId)
    ...
    {
        if ($this->currentCycleId) {
            $this->bloodGasResults = BloodGasResultPicu::with('pegawai')
                ->where('monitoring_cycle_id', $this->currentCycleId)
