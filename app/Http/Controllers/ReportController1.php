...
class ReportController extends Controller
{
    ...
    public function printPdf(string $noRawat, string $sheetDate)
    {
        ...
        $allRawRecords = $cycle->records()
            ->with('inputter:nik,nama')
            ->whereBetween('observation_time', [$cycle->start_time, $cycle->end_time])
            ->orderBy('observation_time', 'asc')
            ->get();
        ...
        $hemodynamicChartConfig = [
            ...
            'data' => [
                ...
                'datasets' => [
                    ...
                    [
                        'label' => 'Suhu (°C)',
                        'data' => $suhuData,
                        'borderColor' => '#1F7A8C', // Teal/Biru Gelap
            ...
            'options' => [
                ...
                'scales' => [
                    'x' => ['grid' => ['display' => true]],
                    'y' => [ // AXIS KIRI: NADI/TENSI/MAP
                        'type' => 'linear',
                        'position' => 'left',
                        'title' => ['display' => true, 'text' => 'Tensi/Nadi/MAP (mmHg/x/mnt)'],
                        'min' => 0,
                        'max' => 200,
                    ],
                    'y1' => [ // AXIS KANAN: SUHU
                        ...
                        'position' => 'right',
                        'title' => ['display' => true, 'text' => 'Suhu (°C)'],
                        'grid' => ['drawOnChartArea' => false], // Tidak perlu grid tambahan
                        'min' => 35,
                        'max' => 41,
                    ],
                ],
